...
print("Fetching Epic...")
epic = get_jira("/rest/api/2/issue/CPG-176")
if epic:
    print(f"=== EPIC CPG-176: {epic['fields']['summary']} ===")
    print(f"Description:\n{epic['fields'].get('description', 'No description')}")
else:
    print("Epic not found.")

jql = '"parent" = CPG-176 OR "Epic Link" = CPG-176'
search_endpoint = f"/rest/api/2/search?jql={urllib.parse.quote(jql)}"
print("\nFetching Children...")
...
if children and 'issues' in children:
    ...
    for issue in children['issues']:
        ...
        print(f"Description:\n{issue['fields'].get('description', 'No description')}\n")
